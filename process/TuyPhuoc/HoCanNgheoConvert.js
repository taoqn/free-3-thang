...
// Paths
const input = path.join(__dirname, '../../data/TuyPhuoc/HoCanNgeo.json');
const output = path.join(__dirname, '../../data/TuyPhuoc/HoCanNgeo.sql');
// Datas
...
Object.keys(rawdata).forEach((idPhuongXa, indexPX) => {
    rawdata[idPhuongXa].forEach((canhan, indexCN) => {
        ...
        if (canhan.QuanHeChuHo + "" === "1") {
            ...
            fileOutput.write(`${sqlHoGiaDinh} ('${IDCaNhan}', ${idPhuongXa}, '${canhan.SoSoHN}', '${IDDoiTuong_HoCanNgheo}');\n`);
        }
        ...
        fileOutput.write(`${sqlThongTinHoTro} ('${uuidv4()}', ${IDNoiDungHoTro}, '${IDDoiTuong_HoCanNgheo}', '${IDCaNhan}');\n`);
    })
});